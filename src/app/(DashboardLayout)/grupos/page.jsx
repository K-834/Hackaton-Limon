    ...
    <div className="container">
      {
        Array.from({ length: numberGroups }, (_, i) => (
          <div onClick={() => setCurrentGroup(i + 1)} key={i} className={`group ${i + 1 === currentGroup ? 'selected' : ''}`} >GRUPO {i + 1}</div>
        ))}
    </div>
  )
    ...
    <div className="cards">
      {
        Array.from({ length: students.length }, (_, i) => (
          <div key={`${students[i].id}${i}`} className="card">
                <div className="image-container">
                  <img src={students[i].urlImg} alt="avatar" />
                </div>
                  <div className="card-body">
                    <h2>{students[i].fullname}</h2>
                    <p>{students[i].hobbies.hobbies}</p>
                    <div className="field">
                        ...
                        <h1 className="icon-label">{students[i].studentCode}</h1>
                    </div>
                    <div className="field">
                        <svg className="icon" viewBox="0 0 512 512">
                            <path d="M437.332 80H74.668C51.199 80 32 99.198 32 122.667v266.666C32 412.802 51.199 432 74.668 432h362.664C460.801 432 480 412.802 480 389.333V122.667C480 99.198 460.801 80 437.332 80zM432 170.667L256 288 80 170.667V128l176 117.333L432 128v42.667z"/>
                        </svg>
                        <h1 className="icon-label">{`${students[i].studentCode}@utp.edu.pe`}</h1>
                    </div>
                  </div>
              </div>
        ))}
    ...
    <PageContainer
      ...
      <DashboardCard title="Grupos">
        <div className="grupos-page">
          {
            numberGroups != null ? listGroups : <Typography variant="h6">Cargando grupos...</Typography>
          }
          <div className="integrantes" >
            ...
            <div className="cards">
              ...
              {
                students.length > 0 ? listStudents : <Typography variant="h6">No hay Integrantes por el momento</Typography>
              }
